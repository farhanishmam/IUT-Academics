...
// - Multicast forwarded by node n2 onto LAN1 and LAN2;
// - Nodes n0, n1, n2, n3, n4, n5, and n6 receive the multicast frame.
// - Node n4 listens for the data in LAN1
// - Node n6 listens for the data in LAN2
...
main(int argc, char* argv[])
{
    ...
    NodeContainer c;
    c.Create(7); //Modified added two new nodes
    ...
    NodeContainer c2 = NodeContainer(c.Get(2), c.Get(5), c.Get(6)); //Modified added a new node container
    ...
    NetDeviceContainer nd2 = csma.Install(c2); // Modified - Added the New Device container for LAN2
    ...
    //Modified added IP address for LAN-2
    ...
    outputDevices.Add(nd1.Get(0));        // (we only need two NetDevice here)
    outputDevices.Add(nd2.Get(0));       //Modified because we have 2 output interfaces
    ...
    //Modified - created a new sink for node n6
    ApplicationContainer sinkD = sink.Install(c2.Get(2)); // Node n6
    // Start the sink
