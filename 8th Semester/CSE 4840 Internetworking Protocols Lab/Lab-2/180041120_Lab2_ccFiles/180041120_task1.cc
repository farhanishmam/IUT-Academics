...
main(int argc, char* argv[])
{
    ...
    NodeContainer c;
    c.Create(6); //Modified - Changed from 5 to 6
    // We will later want two subcontainers of these nodes, for the two LANs
    NodeContainer c0 = NodeContainer(c.Get(0), c.Get(1), c.Get(2));
    NodeContainer c1 = NodeContainer(c.Get(2), c.Get(3), c.Get(4), c.Get(5)); //Modified, added a new node to c1
    ...
    //Modified added a new sink for n5
    ApplicationContainer sinkD = sink.Install(c1.Get(3)); // Node n5
    // Start the sink
