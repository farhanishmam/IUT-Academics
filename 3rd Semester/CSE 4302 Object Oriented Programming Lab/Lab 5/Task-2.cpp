...
void sort_hand(card deck[], int pn)
{
    vector<pair <Suit, int> > v;
    for(int i = (pn-1)*13;i<(pn-1)*13+13;i++)
    {
        int a = deck[i].get_number();
        Suit b = deck[i].get_suit();
        v.push_back(make_pair(b,a));
    }
    sort(v.begin(),v.end());
    for(int i = (pn-1)*13;i<(pn-1)*13+13;i++)
    {
        deck[i].set(v[i].second,v[i].first);
    }
}
...
int main()
{
    ...
    cout<<"Cards of Player-1 is:"<<endl;
    for(j=12; j>-1; j--) //display ordered deck
    {
        deck[j].display();
        cout << " ";
    }
